...
*/
SELECT a.DESCRIP 
FROM ARTICULOS a,FACTURAS f ,CLIENTES c ,LINEAS_FAC lf ,PUEBLOS p ,PROVINCIAS pr
WHERE (a.CODART =lf.CODART) AND (lf.CODFAC =f.CODFAC) AND (f.CODCLI=c.CODCLI) AND (c.CODPUE =p.CODPUE )
AND (p.CODPRO =pr.CODPRO) AND pr.NOMBRE ='%A';
/*
...
WHERE a.CODART =lf.CODART AND f.CODFAC =lf.CODFAC AND f.CODCLI =c.CODCLI AND lf.DTO >10;
...
WHERE c.CODCLI =f.CODCLI AND f.CODFAC =f.CODFAC AND (lf.DTO >10 OR f.DTO >10);
/*
...
FROM ARTICULOS a,FACTURAS f ,LINEAS_FAC lf ,CLIENTES c 
WHERE a.CODART =lf.CODART AND f.CODCLI =c.CODCLI AND c.NOMBRE ='MARIA MERCEDES';
