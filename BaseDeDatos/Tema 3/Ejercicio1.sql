...
SELECT CODFAC,FECHA,IVA,DTO*2 AS "descuento" FROM FACTURAS WHERE IVA=0 ;
...
SELECT DESCRIP,STOCK_MIN, STOCK_MIN-STOCK AS "FALTANTES PARA STOK MAX"
...
SELECT CODART,STOCK_MIN,DESCRIP
...
FROM CLIENTES
	...
	SELECT NOMBRE
	FROM PROVINCIAS WHERE UPPER(NOMBRE) LIKE '_O%';
...
<--Las fechas son muy confusas y no las entiendo)
	SELECT CODFAC,FECHA FROM FACTURAS WHERE CODCLI BETWEEN 50 AND 100
	AND (EXTRACT(YEAR FROM SYSDATE)=1)=EXTRACT(YEAR FROM FECHA);

	SELECT CODFAC,FECHA FROM FACTURAS WHERE CODCLI BETWEEN 50 and 100 and
	'08'=TO_CHAR(FECHA,'YY');
...
<--LAs fechas me dan fallo>
	SELECT CODFAC ,FECHA FROM FACTURAS WHERE TO_CHAR(FECHA,'MM-YY')='06-2004';
	SELECT CODFAC,FECHA  FROM FACTURAS WHERE TO_DATE('01-06-04','DD-MM-YY')<=FECHA AND TO_DATE('30-06-04','DD-MM-YY');
...
<--Me da fallo>
	SELECt CODFAC,FECHA FROM TO_CHAR(FECHA,'MM-YYYY')='06-2004';
